...
class Cube {
    ...
    private fun CubeCoordinates(xMin:Float, yMin:Float, zMin:Float, xMax:Float, yMax:Float, zMax:Float):FloatArray {
    ...
    constructor(xMin:Float, yMin:Float, zMin:Float, xMax:Float, yMax:Float, zMax:Float) {
        ...
        val vbb: ByteBuffer = ByteBuffer.allocateDirect(CubeCoordinates(xMin, yMin, zMin, xMax, yMax, zMax).size * 4)
        vbb.order(ByteOrder.nativeOrder()) // Use native byte order
        vertexBuffer = vbb.asFloatBuffer() // Convert from byte to float
        vertexBuffer?.put(CubeCoordinates(xMin, yMin, zMin, xMax, yMax, zMax)) // Copy data into buffer
