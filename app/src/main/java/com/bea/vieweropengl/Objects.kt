...
class Objects {
    ...
    constructor(xMin:Float, yMin:Float, zMin:Float, xMax:Float, yMax:Float, zMax:Float) {
        ...
        val vbb: ByteBuffer = ByteBuffer.allocateDirect(ObjectCoordinates(xMin, yMin, zMin, xMax, yMax, zMax).size * 4)
        vbb.order(ByteOrder.nativeOrder()) // Use native byte order
        vertexBuffer = vbb.asFloatBuffer() // Convert from byte to float
        vertexBuffer?.put(ObjectCoordinates(xMin, yMin, zMin, xMax, yMax, zMax)) // Copy data into buffer
